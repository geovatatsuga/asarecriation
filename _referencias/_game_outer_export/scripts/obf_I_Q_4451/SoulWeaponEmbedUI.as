package obf_I_Q_4451
{
   ...
   import playerUI.SoulWeaponEmbedUIMC;
   
   public class SoulWeaponEmbedUI extends SoulWeaponEmbedUIMC implements IIconItemUI
   {
      ...
      public function SoulWeaponEmbedUI()
      {
      ...
      private function obf_o_a_1923(param1:Event) : void
      {
         ...
         var _loc5_:int = 120;
         _loc4_.addTipInfo("<p align=\'center\'>" + DiversityManager.getString("SoulWeaponEmbedUI","gemType",[JSONUtil.getInt(_loc2_,["properties","embedGemLevel"])]) + "</p>",_loc5_,0);
      ...
      public function pushEquipIn(param1:IconItemEquip) : void
      {
         ...
         if(_loc2_.subtype != GameItemSubtype.obf_s_2_3291)
         {
            return;
         }
         this.clearContent();
      ...
      public function obf_m_C_3806(param1:String, param2:int) : void
      {
         ...
         WindowManager.showConfirmBox(_loc5_ + "\n" + DiversityManager.getString("SoulWeaponEmbedUI","confirm_Embed",[obf_7_6_4416.getItemLinkA(param1,"")]),this.confirmEmbed,{
            "gemIndex":_loc4_,
            "itemIndex":param2
         });
      }
      ...
      private function selectHole() : void
      {
         ...
         txtChance.text = DiversityManager.getString("SoulWeaponEmbedUI","txtChance") + Number(_loc3_.chance * 100).toFixed(2) + "%";
         var _loc6_:int = 0;
         ...
         for(_loc4_ in _loc7_)
         {
            ...
            if(_loc5_)
            {
               ...
               if(obf_K_e_3075.playerBagUI.getItemCount(_loc4_) < _loc3_.costItems[_loc4_])
               {
         ...
         if(JSONUtil.getInt(_loc3_,["costGold"]) > 0)
         {
            pointGold.addChild(obf_a_f_2935.getGoldDisplay(JSONUtil.getInt(_loc3_,["costGold"])));
         }
         this.action = this.obf_N_p_3608;
         cmdAction.label = DiversityManager.getString("SoulWeaponEmbedUI","cmdHole");
      ...
      private function selectDisembed(param1:String, param2:int) : void
      {
         ...
         cmdAction.label = DiversityManager.getString("SoulWeaponEmbedUI","cmdDisembed");
         cmdAction.visible = true;
      }
      ...
      private function doAction(param1:Event) : void
      {
         ...
         if(this.action == this.obf_N_p_3608)
         {
            ...
            _loc4_ = JSONUtil.getInt(_loc3_,["holeCostList",this.equipInfo.equipmentInfo.holeNumber,"costGold"]);
            if(_loc4_ > 0 && GameContext.localPlayer.fullInfo.gold < _loc4_)
            {
               obf_K_e_3075.showTipInfo(DiversityManager.getString("CommonPrompt","noMoreGold"));
               return;
            }
            _loc5_ = obf_A_V_935.getHoleCost(_loc3_.planId,this.equipInfo.equipmentInfo.holeNumber);
            if(!GameItemManager.itemCanHole(this._equipBag.haveIconItem.itemCode) || !_loc5_)
            {
               obf_K_e_3075.showTipInfo(DiversityManager.getString("HoleUI","error_EquipCantHole"));
               return;
            }
            for(_loc6_ in _loc5_.costItems)
            {
               if(!obf_K_e_3075.playerBagUI || obf_K_e_3075.playerBagUI.getEqualItemCount(_loc6_) < _loc5_.costItems[_loc6_])
               {
         ...
         else if(this.action == this.obf_L_5_3704)
         {
            ...
            _loc7_ = GameItemManager.getItemConfig(this.selectedGemCode);
            if(!_loc7_)
            {
               return;
            }
            _loc8_ = obf_A_V_935.getDisembedCost(JSONUtil.getInt(_loc7_,["properties","gemLevel"]));
            if(!_loc8_ || obf_K_e_3075.playerBagUI.getItemCount(_loc8_.costItemCode) < JSONUtil.getInt(_loc8_,["costItemNum"]))
            {
      ...
      private function setGemBags(param1:GameItemFullInfo) : void
      {
         ...
         while(_loc5_ < this.obf_m_o_2112.length)
         {
            ...
            if(!_loc6_.haveIconItem)
            {
               if(_loc6_.isLocked)
               {
                  DiversityManager.setTextField(_loc7_.txt,"SoulWeaponEmbedUI","gemPrompt_Hole",null,true);
               }
               else
               {
                  DiversityManager.setTextField(_loc7_.txt,"SoulWeaponEmbedUI","gemPrompt_Embed",null,true);
               }
            }
            _loc5_++;
         }
      ...
      {
         DiversityManager.setTextField(txtDeyEquip,"SoulWeaponEmbedUI","txtSoulWeapon");
         DiversityManager.setTextField(txtDeyMaterial,"SoulWeaponEmbedUI","txtMaterial");
         DiversityManager.setTextField(txtDeyPrompt,"SoulWeaponEmbedUI","txtPrompt1");
         DiversityManager.setTextField(txtDeyGroup,"SoulWeaponEmbedUI","txtGroup");
         DiversityManager.setTextField(txtDeyGroupPrompt,"SoulWeaponEmbedUI","txtGroupPrompt");
         DiversityManager.setTextField(txtDeyGold,"SoulWeaponEmbedUI","txtDeyGold");
      }
