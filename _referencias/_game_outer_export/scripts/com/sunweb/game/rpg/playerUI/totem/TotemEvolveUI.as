package com.sunweb.game.rpg.playerUI.totem
{
   ...
   import playerUI.TotemEvolveUIMC;
   
   public class TotemEvolveUI extends TotemEvolveUIMC implements IIconItemUI
   {
      ...
      {
         cmdEvolve.addEventListener(MouseEvent.CLICK,this.obf_v_G_2673);
      }
      
      private function removeListener() : void
      {
         cmdEvolve.removeEventListener(MouseEvent.CLICK,this.obf_v_G_2673);
      }
      
      private function obf_v_G_2673(param1:Event) : void
      {
         var _loc2_:String = null;
         if(this.obf_9_9_3410 <= 0)
         {
            obf_K_e_3075.showTipInfo(DiversityManager.getString("TotemEvolveUI","noActivation"));
            return;
         }
         if(this._currentLv < this.obf_b_6_3110["stage"][this.obf_9_9_3410 - 1]["level"].length)
         {
            obf_K_e_3075.showTipInfo(DiversityManager.getString("TotemEvolveUI","cantmaxLv"));
            return;
         }
         if(this.gold > GameContext.localPlayer.fullInfo.gold)
         {
            obf_K_e_3075.showTipInfo(DiversityManager.getString("TotemEvolveUI","noGold"));
            return;
         }
         for(_loc2_ in this.itemObj)
         {
            ...
            {
               obf_K_e_3075.showTipInfo(DiversityManager.getString("TotemEvolveUI","noSuff"));
               return;
            }
         }
         obf_J_w_1685.sendUpLv(obf_K_e_3075.totemBoxUI.list.selectedIndex,2);
      }
      ...
      public function obf_0_4_u_689(param1:int, param2:int) : void
      {
         ...
         if(this.obf_9_9_3410 <= 0)
         {
            txtDayAttr.text = DiversityManager.getString("TotemEvolveUI","noAct");
            txtAttr.text = _loc3_;
            ...
            this.itemObj = JSONUtil.getObject(this.obf_b_6_3110["stage"][0],["upItem"]);
            this.gold = JSONUtil.getInt(this.obf_b_6_3110["stage"][0],["upGold"]);
            cmdEvolve.enabled = false;
         }
         else
         {
            txtDayAttr.text = DiversityManager.getString("TotemEvolveUI","txtDayAttr");
            ...
            txtAttr.text = _loc3_;
            txtNameAttr.text = _loc4_;
            _loc12_ = JSONUtil.getObject(this.obf_b_6_3110["stage"][this.obf_9_9_3410],["level",0,"attr"]);
            for(_loc13_ in _loc12_)
            {
            ...
            this.itemObj = JSONUtil.getObject(this.obf_b_6_3110["stage"][this.obf_9_9_3410],["upItem"]);
            this.gold = JSONUtil.getInt(this.obf_b_6_3110["stage"][this.obf_9_9_3410],["upGold"]);
            txtNextAttr.text = _loc5_;
            txtNameNextAttr.text = _loc6_;
            cmdEvolve.enabled = true;
         }
         if(this.obf_9_9_3410 >= this.obf_b_6_3110["stage"].length)
         {
            cmdEvolve.enabled = false;
         }
         if(this.obf_9_9_3410 > 0 && !this.obf_b_6_3110["stage"][this.obf_9_9_3410])
         {
            txtNextDayAttr.text = DiversityManager.getString("TotemEvolveUI","maxEvolve");
         }
         else
         {
            txtNextDayAttr.text = DiversityManager.getString("TotemEvolveUI","txtNextDayAttr");
         }
      ...
      {
         DiversityManager.setTextField(txtNextDayAttr,"TotemEvolveUI","txtNextDayAttr");
         cmdEvolve.label = DiversityManager.getString("TotemEvolveUI","cmdEvolve");
      }
